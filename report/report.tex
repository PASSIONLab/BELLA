\title{Long Read Assembly\\
\large Existence and Unique existence of k-mers in the genome}
\author{
        Giulia Guidi\\
        \small giulia.guidi@mail.polimi.it\\
        \small Dipartimento di Elettronica, Informazione e Bioingegneria\\
        \small Politecnico di Milano, Milan, Italy
   	   }
\date{\today}
...
Furthermore, the Pacific Bioscience techonology, considered in this work, is prone to an error rate equal to 10\%.\\
...
This report includes a brief explaination of the proposed approach to compute the analysis in Section \ref{code}, while in Section \ref{results} the obtained results are shown.
...
As said in Section \ref{code}, the \emph{GroupID} variable represents the occurrence of a k-mer in the initial k-mer set. 
...
Regarding the presented plots, for all of them the x-axis is based on the \emph{GroupID}, while the y-axis represents the percetage of k-mers existence (Figure \ref{fig:E}), unique existence (Figure \ref{fig:U}) in the genome or both (Figure \ref{fig:15}, \ref{fig:17} and \ref{fig:19}).\\
...
A stable 100\% existence can be find starting from \emph{GroupID} equal to 56.\\
...
A stable 100\% existence can be find starting from \emph{GroupID} equal to 40.\\
...
A stable 100\% existence can be find starting from \emph{GroupID} equal to 34.\\
...
In Figure \ref{fig:E} and \ref{fig:U}, percentages of existence and unique existence with the three different k-mer length are compared, respectively.\\
Furthermore, I extracted the matches position in the genome for each group to see if there was evidence of association between a groups set and a portion of the genome.
However, the obtained results show an almost uniform distribution of the matches throughout the genome for all the groups.
%The distribution is shown in Figure \ref{fig:distribution}, the x-axis %represents all the Escherichia Coli genome positions, while the y-axis is %composed by a set of group samples and the matches drawn as circles.
\begin{figure}
    \centering
