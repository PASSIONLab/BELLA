...
As a matter of fact, an error rate equal to 15\% implies that every possible k-mer will be seen at least once, given enough depth (sequencer parameter).
...
The histogram in \Cref{fig:hist} shows the percentage of unique, non-genomic and repeated k-mers over the total amount of k-mers belonging to a certain frequency. 
...
The first step consists in the creation of a sparse matrix where row are represented by the reads and columns by the set of reliable k-mers, \myworries{insert figure}.
...
Consequently, we create the transpose of the first one obtaining a k-mers-by-reads matrix.

After that, exploiting the column-by-column algorithm we multiply these two sparse matrices obtaining a resulted matrix allows us to keep track of all the read pairs that share some k-mers and the corresponding positions on both the reads, \myworries{insert figure}.
