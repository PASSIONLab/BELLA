...
public:
   ...
   virtual bool compute_optimal_parameters()
   {
	...
	ABAB: (is this # distinct elements?)
      */
...
public:
   ...
   inline bloom_filter& operator = (const bloom_filter& f)
   {
      if (this != &f)
      {
        // avoids segfault on empty filter 
        if(raw_table_size_>0)
            delete[] bit_table_;
         salt_count_ = f.salt_count_;
         table_size_ = f.table_size_;
         raw_table_size_ = f.raw_table_size_;
   ...
   inline void insert(const T& t)
   {
      ...
      // ABAB: what happens if you reinterpret_cast a class to bytes?
   ...
   size_t storage()	// return memory usage in bytes
   {
	return (sizeof(bloom_type) * salt_.size()) + raw_table_size_;
   }
