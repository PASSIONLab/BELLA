...
SUMMARY="${MYPATH}/bella-pipeline-ecsample-summary-4.csv"
touch ${SUMMARY}
...
MYTEMP="${SCRATCH}/israt-kmer/BELLA/pipeline-tmp-summary-2.txt"
touch ${MYTEMP}

if [ ${WINDOW} == "0" ]; then
	# run bella (need input for minimizer and synchmer)
	${BELLA} -f ${INPUT} -z -k ${KSIZE} -o ${NAME} -c ${DEPTH} -x ${XDROP} -l ${LOWER} -u ${UPPER} -t >> ${MYTEMP}
else
	${BELLA} -f ${INPUT} -z -k ${KSIZE} -o ${NAME} -w ${WINDOW} -c ${DEPTH} -x ${XDROP} -l ${LOWER} -u ${UPPER} -t >> ${MYTEMP}
fi
